...
## 実際の力の伝達とラケット・ガット・ガット張りの特性

上のモデルでは、ラケットとガット面とを完全な剛体とし、それが一瞬だけシャトルに接して力を伝達することを想定している。

しかしながら、実際には、短いながらある期間、ラケットはシャトルと接し、力を与え、その力の積分がシャトルに与えられる力の総量である。

また、ラケットとガットは、シャトルと接したときに変形し、その変形を復元しようとする力がシャトルに力を与える。

実際、ラケットを全く動かさないときにも、シャトルが跳ね返る力のはその変形・復元のせいである。

ラケットとそこに張られたガットが作る変形・反発力は、ラケットの特性・ガットの特性・ガット針の強さを合わせて決まる。

例えば、ガットが全く無変形のとき、変形するのはラケットフレームのみになる。

逆に、無変形のラケットの場合は、ガットが作る網目の面の変形のみとなる。

ガットは、糸としての変形しやすさの要素と、それを格子状に張ったときに、格子面全体としての変形しやすさの要素とがある。
両者は無関係ではないが、それなりに独立した要素である。

他方、どのようなガットを選ぶか、それをどのような強さで張るか、という２つの要因は、選択可能である。
この２要因とラケットの物理特性とを合わせた３要因によって、シャトルへの力の伝達具合が決定される。

衝撃吸収性は、ガット面がシャトルとぶつかることで、すくなくともシャトルはある意味で静止状態になるが、そのためには力を要する。この力のうち身体に伝わる部分が衝撃である。
この力のすべてが身体に伝わらず、ラケットの変形・ガット面の変形によって減じられる。これが衝撃吸収性である。

反発性は、ラケットとガット面が吸収した衝撃のうち、シャトルに反作用的に戻される成分の大きさに関する指標である。

この衝撃吸収・反発はラケットとガット面の物理特性で決まってしまうので、基本的にコントロール不能である。
そのコントロール性がよいというとき、それは２つの要素からなるだろう。

一つ目の要素は、シャトルに伝達する反発力の方向と大きさを、身体側の微妙な動きで変化しやすいことに関するものである。
基本的には、シャトルとガット面との接触時間が長い方がそのような操作をする能力は高くなる。また、接触面が広い方がその能力は高くなる。
また、操作するためには、シャトルからガット面・ラケットを通じて身体側に伝わる力の覚知が適量であって、そこに再現性があることも重要である。
したがって、衝撃吸収はしつつ、吸収しすぎないこと、また、吸収するにしても、シャトルがガット面とどのような物理的接触状態なのかについての情報が乱雑項によってかく乱されにくいような衝撃吸収であることが適切である。

二つ目の要素は、一つ目の要素でも述べた乱雑項に関するものである。
衝撃吸収・身体側への伝達においても乱雑項の作用があり、それが少ないことがコントロール性の高さと関連したが、反発力にも乱雑項は生じうる。これが小さいことがコントロール性を高める。
ガット面とシャトルとの接触面が広いと、各点で生じうる乱雑項を積分したときに、その総和が0周辺によく収束するのに対し、接触面が狭いと、平均は0であっても、その分散は大きくなる。
接触時間の長短も同様の理由で長い方が乱雑項総量の分散は小さくなる。
...
# ラケットの法線単位ベクトル
P0 <- my.unit.vec(c(1,0,-0.05))
# 力ベクトルの法線ベクトル
F0 <- my.unit.vec(c(1,0,-0.05))
# 与える力
f0 <- 40000
```
...
# ラケットの法線単位ベクトル
P0 <- my.unit.vec(c(1,0,-0.65))
# 力ベクトルの法線ベクトル
F0 <- my.unit.vec(c(1,0,-0.65))
# 与える力
f0 <- 120000
```
...
スマッシュとほとんど同じで、でも、ネットを超えたら失速させたい。

失速させる分、力を抜くが、それに加えて、ラケット面を少し上にするとか、ヒットを斜めにするとかする必要がある。
...
# ラケットの法線単位ベクトル
P0 <- my.unit.vec(c(1,0,-0.35))
# 力ベクトルの法線ベクトル
F0 <- my.unit.vec(c(1,0,-0.35))
# 与える力
f0 <- 15000
```
